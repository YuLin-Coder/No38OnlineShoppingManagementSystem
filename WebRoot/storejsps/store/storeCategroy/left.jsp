 ...
 * 1. 对象名必须与第一个参数相同！
   2. 第二个参数是显示在菜单上的大标题
 */
var bar = new Q6MenuBar("bar", "${store.sname}");
$(function() {
...
<%-- <c:forEach items="${parents}" var="parent">   --%>
  <c:forEach items="${parents.children}" var="child">
	bar.add("${parents.cname}", "${child.cname}", "/shopping/store/StoreGoodsServlet?method=findByCategory&cid=${child.cid}&sid=${store.sid}", "body");
  </c:forEach>
<%--  </c:forEach>  --%>
